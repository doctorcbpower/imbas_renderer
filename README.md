...
Imbas (n.) /'imbəs/ — Old Irish for "inspiration" - is a rendering engine for transforming raw astrophysical simulation data into high-fidelity visual maps. It reads particle snapshots in Gadget binary or HDF5 format (e.g. GADGET4, SWIFT, Arepo) and produces PNG images with full control over colourmap, opacity, density scaling, and SPH kernel smoothing. It supports single-frame renders, zoom sequences, and rotation animations, as well as interpolations between frames. It is parallelised with OpenMP and optionally MPI.
